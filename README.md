...
This project consists of building a *[Three.JS](http://threejs.org/)*  based   **Audio Visualizer**,  like those of modern media player applications, to be integrated in a music player built on top of the *[Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)*. 

The idea has its roots deep in my childhood, when I was exploring the digital world on my first desktop computer, when for the first time I was looking through the visualizations of my media player, wondering and curious about how it works and how they build that. 

And here we are... Now I have (hope almost) all the ingredients to build a good one, by myself. About the power, design and functionalities of the current audio standard for the web - *Web Audio API* - I had recently read in some interesting articles on *[Medium](https://medium.com/)*. I wanted to build something that I could use in the future, something practical, not just a 3D visualization. So, the intent was to create a web audio player running on the browser (nowadays almost all the computers have a browser) , a good one, where you can easily **drag and play** your music, enjoying the nice visualization born in your mind and created with passion. 
...
The project was designed to run with good performance on the **Chrome** browser, but also has good performance on other browsers.
...
Instead, online, on GitHub pages, there is a drop in FPS for the Firefox browser (`25-30 FPS`).
...
No models or animations were imported in this project.
...
-   The project has a Control Panel GUI, where some of the objects' properties can be modified, observing the immediate feedback ( around 15 different actions just on the Control Panel GUI);
...
The Audio Player of the project is based on and inspired by a  *[Wavesurfer.JS](https://wavesurfer-js.org/)*  demo example that I adapted for my project. I chose this library because it's built on top of the  *[Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)*  that I needed for the visualization part of the project.

The changes I made to the original demo consist of:
...
```
 ...
 Next, we want to get the Time-Domain and Frequency-Domain Data (JavaScript arrays):

 The FFT (Fast Fourier Transform) algorithm is used for the discrete Frequency Domain Data. 
...
```javascript
// Add the alpha texture to the material and set it up
...
Note how to create the second sphere, which is identical to the first, by using the  `.clone()`  function of the Three.JS meshes.
...
The renderer is responsible for updating the objects' properties and positions in the scene at each frame, taking the passed time into consideration, under certain conditions (`analyser`  availability,  conditions over data objects (`tdd` and `fft`)). The  `render()`  function updates the cubes'  `scale`  properties on the basis of the `fft`  array data content (Frequency Domain Data after the Fast Fourier Transform algorithm was applied) and the coefficients and parameters updated by the user in the *Control Panel GUI*.

Note that some of the properties are updated outside the renderer when some events occur and are managed by certain handlers. This is the case of the  *Control Panel GUI*  components.

Getting back to the renderer... 

The renderer also updates the *Spiral Spheres'*  y-position based on the (updated) average of the `tdd` array (`tddAvg`) that is normalized with the `offsetUpdateSpeed = 0.23` parameter.  In this way we have a bouncing vibration animation of the *Spiral Spheres* and of the corresponding two point lights added to them. 

The *Particle Systems* and the *Main Sphere* are animated also by updating their  `rotation`  properties using the *cos()* and *sin()*  functions, combined with the  `time` variable  and  using the loop index to make different *Particles Systems* behave differently. 
...
Note also that the  `alpha`  property of the renderer is set to  `false`. This gives a black background in the scene and limits some light effects. 
...
The  `smoothingTimeConstant`  parameter of the  `analyser`  object  can be updated asynchronously in the *Control Panel GUI*  and  it is responsible for how responsive the  `fft`  is  (current `fft` overlapped in average with the latest ones)  -  smooth frequency variation in time.
